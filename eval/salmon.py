...
if __name__ == "__main__":
    ...
    tasks = get_dataset_config_names("slprl/SALMon")
    tasks = [c for c in tasks if not c.startswith("all_")]
    result = {}
    for task in tasks:
        ds = load_dataset("slprl/SALMon", task, split="train")
        ds = ds.cast_column(
            "negative_audio", Audio(sampling_rate=feature_extractor.sampling_rate)
        )
        ds = ds.cast_column(
            "positive_audio", Audio(sampling_rate=feature_extractor.sampling_rate)
        )
    ...
    output_path = os.path.join(
        args.output_dir, "SALMON", args.model_name, "accuracy.json"
    )
